...
const createLimiter = rateLimit({
  ...
  max: 20,
  message: {
    error: 'Too many requests. Max 20 URLs per minute.'
  }
});
      ...
      if (customCode) {
        ...
        if (existing) {
          return res.status(409).json({
            error: `"${cleaned}" is already taken. Try another.`
          });
        }
        ...
        const shortUrl = `${BASE_URL}/${cleaned}`;
